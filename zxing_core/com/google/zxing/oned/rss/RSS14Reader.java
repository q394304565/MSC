...
public final class RSS14Reader extends AbstractRSSReader {
	...
	private void adjustOddEvenCounts(boolean outsideChar, int numModules) throws NotFoundException {
		...
		/*
		 * if (mismatch == 2) { if (!(oddParityBad && evenParityBad)) { throw ReaderException.getInstance(); } decrementOdd = true; decrementEven = true; } else
		 * if (mismatch == -2) { if (!(oddParityBad && evenParityBad)) { throw ReaderException.getInstance(); } incrementOdd = true; incrementEven = true; }
		 * else
		 */if (mismatch == 1) {
			if (oddParityBad) {
				if (evenParityBad) {
