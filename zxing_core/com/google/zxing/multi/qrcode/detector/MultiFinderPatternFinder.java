...
final class MultiFinderPatternFinder extends FinderPatternFinder {
	...
	/**
	 * More or less arbitrary cutoff point for determining if two finder patterns might belong to the same code if they differ less than
	 * DIFF_MODSIZE_CUTOFF_PERCENT percent in their estimated modules sizes.
	 */
	private static final float					DIFF_MODSIZE_CUTOFF_PERCENT	= 0.05f;

	/**
	 * More or less arbitrary cutoff point for determining if two finder patterns might belong to the same code if they differ less than DIFF_MODSIZE_CUTOFF
	...
	private FinderPattern[][] selectMutipleBestPatterns() throws NotFoundException {
		...
		/*
		 * Now lets start: build a list of tuples of three finder locations that - feature similar module sizes - are placed in a distance so the estimated
		 * module count is within the QR specification - have similar distance between upper left/right and left top/bottom finder patterns - form a triangle
		 * with 90° angle (checked by comparing top right/bottom left distance with pythagoras)
		 *
		 * Note: we allow each point to be used for more than one code region: this might seem counterintuitive at first, but the performance penalty is not
		 * that big. At this point, we cannot make a good quality decision whether the three finders actually represent a QR code, or are just by chance
		 * layouted so it looks like there might be a QR code there. So, if the layout seems right, lets have the decoder try to decode.
		 */
